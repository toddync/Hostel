...
const [watch, serve, minify, debug, logVars] = [
	"--watch",
	"--serve",
	"--minify",
	"--debug",
	"--log-vars",
].map((s) => process.argv.includes(s));
const debug_console_log = (args, returnIndex = 0) => (
	debug && console.log(...args), args[returnIndex]
);
...
function findPages(dir = ".", sink = []) {
	...
	const svelteFiles = files.filter(
		(f) => f.endsWith(".svelte") && statSync(join(dir, f)).isFile(),
	);
...
const _zId_prefix = `z_placeholder_${Math.floor(
...
const _zReplacer = (s) =>
	debug_console_log(
		[
			"z-replace:",
			s,
			`"${_zId_prefix}${Buffer.from(s).toString("base64")}"`,
		],
		2,
	);
...
const zPlaceholderReplacer = (content) =>
	content
		?.replace(/\#\{\s*\w+\s*\}/gs, _zReplacer) // #{ key }
...
function layoutFor(path, content = {}) {
	...
	if (!path && layoutFor.cache[defaultKey])
		return layoutFor.cache[defaultKey];
...
<html>
  ...
  <body>
	...
	return (layoutFor.cache[path || defaultKey] = ({ js, css }) => {
		...
		cssVarsComments.data = cssVars.length
			? `--- CSS z-vars --- \n${cssVars.join("\n")}`
			: "";
		jsVarsComments.data = jsVars.length
			? `--- JS z-vars --- \n${jsVars.join("\n")}`
			: "";
...
(async () => {
	...
	if (watch) {
		...
		function changeListener(path, stats, type, watcher) {
			...
			else if (svelteFile && type === "unlink")
				pagesPaths.delete(resolve(path));
		...
		const watcher = chokidar
			.watch(".", {
				ignored: (s) =>
					ignorePath.has(s) || ignorePath.has(join("./", s)),
				ignoreInitial: true,
			})
			.on("change", (path, stats) =>
				changeListener(path, stats, "change", watcher),
			)
			.on("add", (path, stats) =>
				changeListener(path, stats, "add", watcher),
			)
			.on("unlink", (path, stats) =>
				changeListener(path, stats, "unlink", watcher),
			)
			.on("ready", () => {
				console.log(
					`watching ${sum(
						Object.values(watcher.getWatched()).map(
							(t) => t.length,
						),
					)} files/dirs for changes`,
				);
				watcherReady = true;
			})
